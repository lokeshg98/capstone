...
import { Calendar, MessageSquarePlus, Trash2, Clock, RefreshCw } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="h-14 border-b border-gray-200 px-6 flex items-center gap-4">
        ...
        <div className="flex gap-1 ml-auto">
          {(['scheduled', 'welcome'] as Tab[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
            ...
            >
              {t === 'scheduled' ? 'Scheduled Posts' : 'Welcome Message'}
            </button>
          ))}
        </div>
      </div>
