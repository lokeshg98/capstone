...
public class CommunityAgentTools {
    ...
    @Tool("Recall semantically similar notes from this user's past bot conversations in this workspace.")
    public String recallPastConversations(String query) {
        ...
        try {
            ...
            List<String> hits = longTermMemoryService.search(ctx.workspaceId(), ctx.userId(), vec, k);
            if (hits.isEmpty()) {
    ...
    @Tool("Summarize a discussion thread (root message UUID). Returns bullets: TL;DR, decisions, open questions.")
    public String summarizeThread(String threadRootId) {
        ...
        String transcript = buildThreadTranscript(ctx, threadRootId);
        if (transcript.startsWith("Invalid") || transcript.contains("not found")
                || transcript.contains("another workspace")) {
            return transcript;
        }
        if (transcript.lines().count() < 2) {
            return "Thread is too short to summarize meaningfully.";
        }
        UserMessage userMsg = UserMessage.from(
                "Summarize this workspace thread for busy moderators. Use markdown bullets with sections: "
                        + "TL;DR, Key decisions, Open questions, Risks. Be faithful to the transcript only.\n\n"
                        + transcript
        );
        return chatModel.chat(userMsg).aiMessage().text();
    }
