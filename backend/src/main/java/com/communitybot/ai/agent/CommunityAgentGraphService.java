...
public class CommunityAgentGraphService {
    ...
    private static final String SYSTEM_MEMBER = """
            ...
            Past Ask Bot conversations are stored in a vector database (pgvector) and relevant
            excerpts are injected into your prompt automatically. You can also call recallPastConversations
            or searchFaqDocuments for more context.
    ...
    private static final String SYSTEM_MODERATOR = """
            ...
            Past Ask Bot conversations are stored in pgvector and auto-retrieved for context.
    ...
    void initGraph() throws GraphStateException {
        ...
        CompiledGraph<AgentExecutor.State> publicAgent = compileAgent(
                chatModel, publicTools, SYSTEM_PUBLIC);
        CompiledGraph<AgentExecutor.State> memberAgent = compileAgent(
                chatModel, workspaceTools, SYSTEM_MEMBER);
        CompiledGraph<AgentExecutor.State> moderatorAgent = compileAgent(
                chatModel, workspaceTools, SYSTEM_MODERATOR);
        CompiledGraph<AgentExecutor.State> channelAgent = compileAgent(
                chatModel, workspaceTools, SYSTEM_CHANNEL);
    ...
    public String runSync(String question, String memoryId) {
        ...
        Map<String, Object> input = Map.of("messages", question);
        RunnableConfig config = RunnableConfig.builder()
                .threadId(memoryId == null ? "anon" : memoryId)
                .build();

        try {
            Optional<AgentExecutor.State> state = orchestrator.invoke(input, config);
            if (state.isEmpty()) {
                return "I couldn't produce an answer. Please try again.";
            }
            return state.get().finalResponse()
                    .filter(s -> !s.isBlank())
                    .orElseGet(() -> lastAiText(state.get()));
        } catch (Exception e) {
            log.error("LangGraph4j agent run failed", e);
